...
void setup() {
  ...
  for (int i=0; i < jsonData.size (); i++) {
    ...
    JSONObject datum = jsonData.getJSONObject(i);
    
    long tmp = datum.getLong("dt");
    Date t = new Date(tmp*1000L); //<>//
    SimpleDateFormat sdf = new SimpleDateFormat("MM-dd-yyyy");
    times[i] = sdf.format(t);
    clouds[i] = datum.getInt("clouds");
    minTemps[i] = datum.getJSONObject("temp").getFloat("min");
    maxTemps[i] = datum.getJSONObject("temp").getFloat("max");
  }
